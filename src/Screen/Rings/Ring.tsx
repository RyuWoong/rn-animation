...
export const Ring = ({
  ...
  const clip = useMemo(() => {
    ...
    const innerCirlce = Skia.Path.Make();
    innerCirlce.addCircle(center.x, center.y, size / 2 - strokeWidth);
    return Skia.Path.MakeFromOp(outerCircle, innerCirlce, PathOp.Difference);
  }, [center.x, center.y, size, strokeWidth]);

  const fullPath = useMemo(() => {
    const path = Skia.Path.Make();
    const fullRevolutios = Math.floor(totalProgress);
    for (let i = 0; i < fullRevolutios; i++) {
